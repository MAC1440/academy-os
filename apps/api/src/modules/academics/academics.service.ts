...
export class AcademicsService {
  ...
  async createAcademicGroup(dto: CreateAcademicGroupDto, actor: string) {
    ...
    return this.createUnique('AcademicGroup', dto, actor, () => this.prisma.academicGroup.create({
      data: {
        organizationId: organization.id,
        name: dto.name.trim(),
        code: dto.code?.trim(),
        schoolClasses: { create: dto.schoolClassIds.map((schoolClassId) => ({ schoolClassId })) },
      },
      include: { schoolClasses: { include: { schoolClass: true } } },
    }));
  }

  async updateAcademicGroup(id: string, dto: UpdateAcademicGroupDto, actor: string) {
    const group = await this.academicGroup(id);
    return this.updateUnique('AcademicGroup', id, dto, actor, () => this.prisma.academicGroup.update({
      where: { id: group.id },
      data: {
        ...this.clean(dto),
        ...(dto.status === EntityStatus.ARCHIVED ? { deletedAt: new Date() } : {}),
        ...(dto.status && dto.status !== EntityStatus.ARCHIVED ? { deletedAt: null } : {}),
      },
      include: { schoolClasses: { include: { schoolClass: true } } },
    }));
  }

  async replaceAcademicGroupSchoolClasses(id: string, dto: ReplaceAcademicGroupSchoolClassesDto, actor: string) {
    const group = await this.academicGroup(id);
    await this.verifySchoolClasses(dto.schoolClassIds);
    const existingOfferingCount = await this.prisma.academicOffering.count({
      where: { academicGroupId: group.id, schoolClassId: { notIn: dto.schoolClassIds } },
    });
    if (existingOfferingCount) {
      throw new BadRequestException('Keep every school class currently used by this group on an academic offering');
    }
    const updated = await this.prisma.academicGroup.update({
      ...
      data: {
        schoolClasses: {
          deleteMany: {},
          create: dto.schoolClassIds.map((schoolClassId) => ({ schoolClassId })),
        },
      },
      include: { schoolClasses: { include: { schoolClass: true } } },
    });
    await this.audit(actor, AuditAction.UPDATE, 'AcademicGroupSchoolClasses', group.id, dto);
    return updated;
  }
  ...
  async createOffering(branchId: string, dto: CreateAcademicOfferingDto, actor: string) {
    ...
    const offering = await this.createUnique('AcademicOffering', dto, actor, () => this.prisma.academicOffering.create({ data: { branchId, offeringType: dto.offeringType, schoolClassId: source.schoolClassId, courseId: source.courseId, academicGroupId: source.academicGroupId, sectionName: source.sectionName, offeringKey: source.offeringKey }, include: this.offeringInclude }));
    return offering;
  }
  async updateOffering(branchId: string, offeringId: string, dto: UpdateAcademicOfferingDto, actor: string) {
    ...
    if (dto.sectionName !== undefined || dto.academicGroupId !== undefined) {
      ...
      const sectionName = dto.sectionName?.trim() ?? offering.sectionName ?? undefined;
      if (offering.schoolClass.sectionsEnabled && !sectionName) throw new BadRequestException('A section is required for this school class');
      const academicGroupId = dto.academicGroupId ?? offering.academicGroupId ?? undefined;
      if (academicGroupId) await this.verifyAcademicGroupForSchoolClass(academicGroupId, offering.schoolClassId!);
      offeringKey = this.offeringKey(offering.offeringType, offering.schoolClassId!, sectionName, academicGroupId);
    }
  ...
  private readonly offeringInclude = { schoolClass: true, course: true, academicGroup: true, subjects: { include: { subject: true } }, teachers: { include: { staffProfile: { include: { user: { select: { id: true, fullName: true } } } } } } } satisfies Prisma.AcademicOfferingInclude;
  ...
  private async academicGroup(id: string) { const organization = await this.organization(); const record = await this.prisma.academicGroup.findFirst({ where: { id, organizationId: organization.id, deletedAt: null } }); if (!record) throw new NotFoundException('Academic group not found'); return record; }
  ...
  private async offeringSource(dto: CreateAcademicOfferingDto) {
    if (dto.offeringType === AcademicOfferingType.SCHOOL_CLASS) {
      ...
      if (dto.academicGroupId) await this.verifyAcademicGroupForSchoolClass(dto.academicGroupId, schoolClass.id);
      return { schoolClassId: schoolClass.id, courseId: null, academicGroupId: dto.academicGroupId ?? null, sectionName: sectionName ?? null, offeringKey: this.offeringKey(dto.offeringType, schoolClass.id, sectionName, dto.academicGroupId) };
    }
    if (!dto.courseId || dto.schoolClassId || dto.sectionName || dto.academicGroupId) throw new BadRequestException('A course offering requires only courseId and cannot have a section or academic group');
    const course = await this.course(dto.courseId); return { schoolClassId: null, courseId: course.id, academicGroupId: null, sectionName: null, offeringKey: this.offeringKey(dto.offeringType, course.id) };
  }
  private offeringKey(type: AcademicOfferingType, sourceId: string, sectionName?: string, academicGroupId?: string) { return `${type}:${sourceId}:${sectionName?.trim().toLocaleLowerCase() ?? '-'}:${academicGroupId ?? '-'}`; }
  ...
  private async verifySchoolClasses(ids: string[]) { if (!ids.length) return; const organization = await this.organization(); const count = await this.prisma.schoolClass.count({ where: { id: { in: ids }, organizationId: organization.id, deletedAt: null } }); if (count !== ids.length) throw new NotFoundException('One or more school classes were not found'); }
  private async verifyAcademicGroupForSchoolClass(academicGroupId: string, schoolClassId: string) { const group = await this.prisma.academicGroup.findFirst({ where: { id: academicGroupId, deletedAt: null, schoolClasses: { some: { schoolClassId } } } }); if (!group) throw new BadRequestException('Academic group is not available for this school class'); }
