...
export class OrganizationService {
  ...
  async updateBranch(
    ...
    if (dto.status && dto.status !== EntityStatus.ARCHIVED)
      data.deletedAt = null;
  ...
  async createOperatingHour(
    branchId: string,
    dto: CreateBranchOperatingHourDto,
    actorUserId: string,
  ) {
    ...
    try {
      ...
      if (this.isUniqueViolation(error))
        throw new ConflictException(
          'Operating-hour labels must be unique within a branch',
        );
      throw error;
    }
  }
  ...
  async updateOperatingHour(
    ...
    dto: UpdateBranchOperatingHourDto,
    actorUserId: string,
  ) {
    const branch = await this.getBranch(branchId);
    const operatingHour = await this.prisma.branchOperatingHour.findFirst({
      where: { id: operatingHourId, branchId },
    });
    if (!operatingHour)
      throw new NotFoundException('Branch operating hours not found');
    try {
