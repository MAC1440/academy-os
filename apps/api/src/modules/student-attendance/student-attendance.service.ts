...
export class StudentAttendanceService {
  ...
  async roster(
    ...
    const students = await this.prisma.student.findMany({
      ...
      include: {
        attendance: { where: { attendanceDate: this.date(attendanceDate) } },
      },
  ...
  async save(
    offeringId: string,
    dto: SaveStudentAttendanceDto,
    actorUserId: string,
  ) {
    await this.ensureBranchAccess(offeringId, actorUserId);
    const students = await this.prisma.student.findMany({
      where: {
        id: { in: dto.records.map((record) => record.studentId) },
    ...
    await this.prisma.$transaction(
      dto.records.map((record) =>
        this.prisma.studentAttendance.upsert({
          ...
          update: { status: record.status, markedByUserId: actorUserId },
          create: {
            studentId: record.studentId,
            attendanceDate,
  ...
  private async ensureBranchAccess(offeringId: string, userId: string) {
    ...
    if (!(await this.access.canAccessBranch(userId, offering.branchId)))
