...
const baseQueryWithRefresh: BaseQueryFn<string | FetchArgs, unknown, FetchBaseQueryError> = async (args, api, extraOptions) => {
  ...
  if (!refreshToken) { api.dispatch(signOut()); return result; }
  refreshRequest ??= (async () => {
    const response = await rawBaseQuery({ url: "/auth/refresh", method: "POST", body: { refreshToken } }, api, extraOptions);
    return "data" in response ? response.data as SessionResponse : null;
  })().finally(() => { refreshRequest = null; });
  const refreshed = await refreshRequest;
  if (!refreshed?.accessToken) { api.dispatch(signOut()); return result; }
  api.dispatch(setCredentials(refreshed));
  result = await rawBaseQuery(args, api, extraOptions);
  return result;
};
export const baseApi = createApi({ reducerPath: "api", baseQuery: baseQueryWithRefresh, tagTypes: ["Session", "Organization", "Branch", "Academic"], endpoints: () => ({}) });
